...
def builtin_microcode_display(args):
    s = str(args[0])
    if len(args) == 2:
        if type(arg[1]) is str: s = args[1] + ' ' + s
        else: raise Exception("display expects string as second argument")
    elif len(args) != 1: raise Exception("display expects one or two arguments")
    print(s)
    return args[0]
...
def builtin_microcode_error(args):
    s = str(args[0])
    if len(args) == 2:
        if type(args[1]) is str: s = args[1] + ' ' + s
        else: raise Exception("error expects string as second argument")
    elif len(args) != 1: raise Exception("error expects one or two arguments")
    raise Exception(s)
...
def lookup_(x, e, i):
    if e == None: raise Exception('unbound name: ' + x)
    if x in e[0]:
        return (e[0][x], i)
    else:
...
def cse_microcode_app_i(cmd):
    ...
    if sf['tag'] == 'builtin':
        ...
        if type(last_prm) is dict and last_prm['tag'] == 'rest':
            ...
            args = new_args
        if len(args) > len(prms): raise Exception('too many arguments')
        if len(args) < len(prms): raise Exception('too few arguments')
...
cse_microcode = {
    ...
    ##########################
    'lit': 
    lambda cmd: S.append(float(cmd['val']) if type(cmd['val']) is int else cmd['val']),
    'nam': 
    cse_microcode_nam,
    'unop': 
...
print("memory consumption before gc: " + str(tracemalloc.get_traced_memory()[0]) + " bytes")
gc.collect(2)
print("memory consumption after  gc: " + str(tracemalloc.get_traced_memory()[0]) + " bytes")
tracemalloc.stop()
