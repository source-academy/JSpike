...
function equal(xs, ys) {
  ...
  // stream_tail and stream added here, instead of providing as builtin

  function stream_tail(x) {
      return tail(x)();
  }

  function $stream(i, n, xs) {
      return i >= n
             ? null
          : pair(xs[i], () => $stream(i + 1, n, xs));
  }
			  
  function stream(...xs) {
      return $stream(0, array_length(xs), xs);
  }
