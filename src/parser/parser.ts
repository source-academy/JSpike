...
import { parse, display_list, display, head, tail,
    list, map, error, set_head, is_null, stringify,
    length, pair, append, accumulate
} from "sicp";
...
type LogicalComposition = { tag: "log"; 
                         sym: string;
                         frst: SyntaxTree; 
                         scnd: SyntaxTree }
type BinaryOperation = { tag: "binop"; 
                         sym: string;
                         frst: SyntaxTree; 
                         scnd: SyntaxTree }
type ArrayAccess = { tag: "arr_acc"; 
                         arr: SyntaxTree;
                         ind: SyntaxTree }
type ArrayAssignment = { tag: "arr_assmt"; 
                         arr: SyntaxTree;
                         ind: SyntaxTree;
                         expr: SyntaxTree }
type LiteralArray = { tag: "arr_lit"; 
                      elems: [SyntaxTree] }
type UnaryOperation = { tag: "unop"; 
                        sym: string;
                        frst: SyntaxTree }
type Decl = string | { tag: "rest"; sym: string }
type Lambda = { tag: "lam"; prms: [Decl]; body: SyntaxTree }
type Sequence = { tag: "seq"; stmts: [SyntaxTree] }
type ConditionalExpression = { tag: "cond_expr"; 
                     pred: SyntaxTree;
                     cons: SyntaxTree;
                     alt: SyntaxTree }
type ConditionalStatement = { tag: "cond_stmt"; 
                     pred: SyntaxTree;
                     cons: SyntaxTree;
                     alt: SyntaxTree }
type While = { tag: "while"; 
               pred: SyntaxTree;
               body: SyntaxTree }
type For = { tag: "for"; 
             init: SyntaxTree;
...
type Let = { tag: "let";
             sym: string;
             expr: SyntaxTree }
type Assignment = { tag: "assmt";
                    sym: string;
                    expr: SyntaxTree }
type Const = { tag: "const";
             sym: string;
             expr: SyntaxTree }
type Function = { tag: "fun";
                  sym: string;
                  prms: [Decl];
                  body: SyntaxTree }
type Return = { tag: "ret";
...
function parameters(xs) {
    return map(x => head(x) === "rest_element"
                    ? { tag: "rest",
                        sym: head(tail(head(tail(x)))) }
                    : head(tail(x)),
               xs);
}
